...
/**
 * 递归遍历list 返回name path &
 * @param list
 * @returns ${value}__RC_CASCADER_SPLIT__${nextValue}
 */
export function treeNamePaths(
  list: any,
  fieldNames: { label: string; value: string; children: string }
) {
  const arr: any = [];
  ...
  function eachData(
    ...
    for (let i = 0; i < arrList.length; i++) {
      let nPointStr = pointStr;
      let nNamePath = namePath;
      if (nPointStr) {
        // 北京市  北京市/北京市
        nPointStr += `__RC_CASCADER_SPLIT__${arrList[i][fieldNames.value]}`;
        nNamePath += `/${arrList[i][fieldNames.label]}`;
      } else {
        nPointStr = arrList[i][fieldNames.value];
        nNamePath = arrList[i][fieldNames.label];
      }
      arrList[i].joinName = nNamePath;
      arrList[i].joinValue = nPointStr;
      arr.push(arrList[i]);
      ...
      if (
        ...
        eachData(arrList[i][fieldNames.children], nPointStr, nNamePath);
      }
    }
  }
  eachData(list);
